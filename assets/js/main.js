...
(() => {
  ...
  (() => {
    ...
    const tick = () => {
      ...
      b.style.setProperty('--bg-scale', (1 + Math.abs(px) * 0.01 + Math.abs(py) * 0.01));
    ...
    addEventListener('pointermove', (e) => {
      ...
      if (!needs) { needs = true; requestAnimationFrame(tick); }
    }, { passive: true });
  })();
  ...
  document.addEventListener('DOMContentLoaded', () => {
    /* карусели */
    try { qsa('.carousel').forEach((el) => new Carousel(el)); }
    catch (e) { console.debug('Carousel init skipped:', e); }

    /* hscroll */
    try { bindHorizontalScroll(); }
    catch (e) { console.debug('bindHorizontalScroll skipped:', e); }

    /* кнопка «наверх» */
    const topBtn = qs('#to-top');
    if (topBtn) {
      const showTop = () => {
        topBtn.classList.toggle('show', (window.scrollY || document.documentElement.scrollTop) > 600);
      };
    ...
    (() => {
        ...
        try {
          if (isNaN(audio.duration)) {
            await new Promise((res) => {
              const ready = () => { audio.removeEventListener('loadedmetadata', ready); res(); };
      ...
      if (audio.dataset.volume) audio.volume = Math.min(1, Math.max(0, Number(audio.dataset.volume)));
